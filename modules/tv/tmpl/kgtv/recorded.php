...
<div id="debug_div" style="display: none"><?php var_dump($sample_record) ?></div>
...
?>
    ...
    <td class="x-recordings"><?php echo t('Show channels') ?>:</td>
...
<tr class="menu">
    ...
    <th class="x-title" colspan="4"><?php             echo get_sort_link('title',           t('Title'))            ?></th>
...
?>
    <td rowspan="2" class="x-pixmap<?php
        if (true){ //$_SESSION['recorded_pixmaps']) { // removed because did not work on iOS
?>"><?php
        ...
        <a class="x-pixmap" href="<?php echo root_url ?>tv/detail/<?php echo $show->chanid, '/', $show->recstartts ?>" title="<?php echo t('Recording Details') ?>"
            ><img src="<?php echo $show->thumb_url(100,0,10) ?>" width="100" height="<?php echo floor(100 / $show->getAspect()); ?>"></a>
<?php   }
        else
            echo ' -noimg">noooo';
?>
    ...
    <td class="x-title" colspan="4"><?php echo '<a href="', root_url, 'tv/detail/', $show->chanid, '/', $show->recstartts, '"'
...
?>
<tr><td colspan="10"><div id="recorded_pager" style="text-align: center;">
<?php
    if ($_SESSION['recorded_paging'] > 0) {
        echo 'Pages - ';
        $total_pages = ceil(count($All_Shows)/$_SESSION['recorded_paging']);
