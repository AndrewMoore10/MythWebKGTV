...
// Set the desired page title
    $page_title = 'KGTV MythTV - '.t('Program Detail').":  $schedule->title";
...
// Custom headers
    ...
    $headers[] = '<script type="text/javascript" src="'.root_url.'js/libs/flowplayer/flowplayer-3.2.12.min.js"></script>';
//    
//    $headers[] = '<script type="text/javascript" src="//ajax.googleapis.com/ajax/libs/jquery/1.10.2/jquery.min.js" ></script>';
...
<!--                        <option value="mov">Quicktime</option>-->
            ...
            <tr>
                ...
                <td><input type="button" value="Cancel" onclick="$j('#exportBG').hide();"/></td>
            </tr>
        </table>
    </div>
    </div>
...
<?php   } ?>
            ...
            <div class="x-pixmap" id="flashbox" style="display:block;">
                <?php
                $mobileFile = $_SERVER['DOCUMENT_ROOT']."/h264xcode/".$program->chanid."_".date( 'YmdHis', $program->recstartts-date('Z') ).".m4v";
              
//                if (file_exists($mobileFile)){
//                 echo "YES $mobileFile";    
//                } 
//                else echo "NO $mobileFile";
                ?>
<?php   if ((setting('WebFLV_on') && !$mobile && !file_exists($mobileFile))) { ?>
<?php       if (file_exists('js/libs/flowplayer/flowplayer-3.2.16.swf')) { ?>
          ...
          <!-- this A tag is where your Flowplayer will be placed. it can be anywhere -->
            ...
            <script>
                flowplayer( 
                    "player",
                    "<?php echo root_url ?>js/libs/flowplayer/flowplayer-3.2.16.swf", {
...
//                                netConnectionUrl: "<?php echo video_url($program, 'flv'); ?>"
//                            },
                            controls: {
                        ...
                        playlist: [
                            ...
                            {
                                url: '<?php echo $program->thumb_url($flv_w,0,10) ?>',
                                scaling: 'orig'
                                },
...
<?php       } elseif (file_exists('modules/tv/MFPlayer.swf')) { ?>
                    ...
                    if ( hasProductInstall && !hasRequestedVersion ) {
                        ...
                        AC_FL_RunContent(
                                ...
                                "flashvars",'file=<?php     echo video_url($program, 'flv');
                                         ?>&still=<?php     echo $program->thumb_url($flv_w,$flv_h,10);
                        ...
                        document.write('<img src="<?php echo $program->thumb_url($flv_w,0,10) ?>"'
                    ...
                    <object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"
                        ...
                        <embed src="<?php echo root_url; ?>tv/MFPlayer.swf" quality="high" bgcolor="#869ca7"
                            ...
                            allowFullScreen="true"
                            FlashVars="file=<?php     echo video_url($program, 'flv');
                                   ?>&still=<?php     echo $program->thumb_url($flv_w,0,10);
                                   ?>&totalTime=<?php echo $program->length;
                                   ?>&width=<?php     echo $flv_w;
                                   ?>&height=<?php    echo $flv_h;
                                   ?>&styles=<?php    echo root_url ?>tv/MFPlayer_styles.swf"
...
<!--                <source src="<?php echo video_url($program, 'webm')?>.webm">-->
    ...
    <div id="HTML5Controls">
        <table id="exportTable" >
            <tr>
                ...
                <td class="lbexport"> <input type="button" onclick="$j('#exportBG').show();" title="Export" Value="Export" /></td>
    <!--            <td class="lbexport"> <input type="button" onclick="return exportVid('<?php echo $program->url ?>', '.mpg')" title="Export" Value="Export" /></td>-->
        ...
        <img src="<?php echo skin_url?>/img/question.png" onclick="$j('#keytooltip').toggle()"/>
        <div id="keytooltip" style="display:none;"><img style="width:500px" src="<?php echo skin_url?>/img/MythKeyboard.png" /></div>
    </div>

    <script type="text/javascript">
    var player = document.getElementById("videoPlayer");
    var label = document.getElementById("playSpeedLabel");
...
//       console.log(player.playbackRate + " hererer : "+ event.which);
       ...
       $j('input[type="text"]').keydown(function(event) {
       if(event.which == 39 ){ // handler for right arrow  
...
<!--            <div class="x-pixmap" id="mobilebox" style="display:none;">
                <video controls preload="none" width="400" height="225" poster="<?php echo $program->thumb_url($flv_w,0) ?>">
                     <source src="/h264xcode/<?php echo $program->chanid ?>_<?php echo date( 'YmdHis', $program->recstartts-date('Z') ) ?>.m4v">
                Your browser does not support the video tag.
                </video>
            </div>-->
            <div class="x-links">
                ...
                <?php if (setting('WebFLV_on') && file_exists('modules/tv/MFPlayer.swf') && false) { ?>
               ...
               <?php 
                    if( file_exists($mobileFile)){ ?>
                <a href="/h264xcode/<?php echo $program->chanid ?>_<?php echo date( 'YmdHis', $program->recstartts-date('Z') ) ?>.m4v">&nbsp;
                    <img src="<?php echo skin_url ?>/img/mobile.png">Mobile</a>
                <?php } else { ?>
<!--                <a href="/h264xcode/<?php echo $program->chanid ?>_<?php echo date( 'YmdHis', $program->recstartts-date('Z') ) ?>.m4v">
                    <?php  ?>
                    Mobile</a>
            ...
            <input type="button" value="Advanced" onclick="$j('.x-jobs').toggle();" style="margin-left: 30px;"/>
            <div class="x-jobs" style="display:none">
<?php
...
<?php   } ?>
    ...
    // Can we perform an accurate duplicate check?
        ...
        <?php } ?>
        <?php if (count($conflicting_shows)) {  ?>
          <tr id="x-conflicts">
              <th><?php echo t('Possible conflicts') ?>:</th></tr>
          <tr><td></td><td>
...
?>
    ...
    <div id="schedule">
        <img class="exit" src="<?php echo skin_url ?>img/close.png" onclick="$j('#schedule').slideUp()"/>
